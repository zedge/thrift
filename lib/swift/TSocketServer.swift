...
public class TSocketServer {
  ...
  public init?(port: Int, protocolFactory: TProtocolFactory, processorFactory: TProcessorFactory) {
    ...
    processingQueue = DispatchQueue(label: "TSocketServer.processing",
                                    attributes: [.concurrent, .qosBackground])
    ...
    if sock != nil {
      ...
      var yes = 1
      setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &yes, UInt32(sizeofValue(yes)))
      ...
      let ptr = withUnsafePointer(&addr) {
        return UnsafePointer<UInt8>($0)
      }
      
      let address = Data(bytes: ptr, count: sizeofValue(addr))
      if CFSocketSetAddress(sock, address) != CFSocketError.success { //kCFSocketSuccess {
